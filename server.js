  ...
  ws.on("message", (raw) => {
    ...
    switch (msg.type) {
      case "create": {
        ...
        console.log(`[ROOM] Đã tạo phòng ${code} (độ khó: ${rooms.get(code).diff}, bản đồ: ${rooms.get(code).mapSize}). Đang chờ người 2...`);
        break;
      }
      case "join": {
        ...
        safeSend(room.host, { type: "start", player: 1, diff: room.diff, mapSize: room.mapSize, room: code });
        safeSend(ws, { type: "start", player: 2, diff: room.diff, mapSize: room.mapSize, room: code });
