...
const VALID_MAP_SIZES = new Set(["small", "medium", "large", "huge"]);
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (raw) => {
    ...
    switch (msg.type) {
      case "ping": {
        // [ĐO ĐỘ TRỄ] Trả thẳng lại ngay cho CHÍNH người gửi (không phải đối thủ)
        // kèm mốc thời gian họ gửi đi, để client tự tính RTT = now - t và suy ra
        // độ trễ 1 chiều, dùng để tự điều chỉnh độ trễ nội suy phía đối thủ
        // (giống cơ chế đo ping trong các game MOBA).
      ...
      case "create": {
        ...
        rooms.set(code, {
          ...
          mapSize: (VALID_MAP_SIZES.has(msg.mapSize) ? msg.mapSize : "small"),
          mode: (msg.mode === "pvp" ? "pvp" : "split"),
        ...
        console.log(`[ROOM] Đã tạo phòng ${code} (độ khó: ${rooms.get(code).diff}, bản đồ: ${rooms.get(code).mapSize}, chế độ: ${rooms.get(code).mode}). Đang chờ người 2...`);
        break;
      }
      case "join": {
        ...
        safeSend(room.host, { type: "start", player: 1, diff: room.diff, mapSize: room.mapSize, mode: room.mode, room: code });
        safeSend(ws, { type: "start", player: 2, diff: room.diff, mapSize: room.mapSize, mode: room.mode, room: code });
      ...
      case "pvpInit": {
        ...
        if (!room) break;
        const other = peerOf(ws, room);
        safeSend(other, Object.assign({}, msg, { type: "pvpInit" }));
        break;
      }
      case "pvpInput": {
        const room = rooms.get(ws.room);
        if (!room) break;
        const other = peerOf(ws, room);
        safeSend(other, Object.assign({}, msg, { type: "pvpInput" }));
        break;
      }
      case "pvpState": {
        const room = rooms.get(ws.room);
        if (!room) break;
        const other = peerOf(ws, room);
        safeSend(other, Object.assign({}, msg, { type: "pvpState" }));
        break;
      }
      case "pvpFoods": {
        const room = rooms.get(ws.room);
        if (!room) break;
        const other = peerOf(ws, room);
        safeSend(other, Object.assign({}, msg, { type: "pvpFoods" }));
        break;
      }
      case "pvpDash": {
        const room = rooms.get(ws.room);
        if (!room) break;
        const other = peerOf(ws, room);
        safeSend(other, Object.assign({}, msg, { type: "pvpDash" }));
        break;
      }
      case "pvpRoundEnd": {
        const room = rooms.get(ws.room);
        if (!room) break;
        const other = peerOf(ws, room);
        safeSend(other, Object.assign({}, msg, { type: "pvpRoundEnd" }));
        break;
      }
      case "pvpRestart": {
        const room = rooms.get(ws.room);
        if (!room) break;
        const other = peerOf(ws, room);
        safeSend(other, { type: "pvpRestart" });
        break;
      }
      case "leaveMatch": {
        const room = rooms.get(ws.room);
